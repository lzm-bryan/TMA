...
import argparse
import configparser
from TMA import make_model, EvidentialLossSumOfSquares, maemis_loss, spline_quantile_loss, EvidentialLoss_encoder
...
clip_value = 5  # 选择一个合适的裁剪值
# read hyper-param settings
parser = argparse.ArgumentParser()
parser.add_argument("--config", default='configurations/TMA.conf', type=str, help="configuration file path")
parser.add_argument("--device", default='cuda:7', type=str, help="choose gpu")
...
graph_signal_matrix_filename = 's' + str(points_per_mhalf) + '_p' + str(num_for_predict) + '_n' + str(
    neighbor_node) + '_m' + str(mask_num) + '_dataSplit.pkl'
graph_signal_matrix_filename = os.path.join(state_basedir, graph_signal_matrix_filename)

folder_dir = 's%d_p%d_n%d_m%d_%s_%d' % (
points_per_mhalf, num_for_predict, neighbor_node, mask_num, model_name, encoder_input_size)
...
def train_main(net, train_loader, val_loader, test_loader, test_target_tensor, _mean, _std, mask_matrix, mask_or):
    torch.save(test_target_tensor.cpu().numpy(), os.path.join('', 'test_target_tensor'))
    ...
    for epoch in range(start_epoch, epochs):
        ...
        for _, batch_data in enumerate(train_loader):
            ...
            # print(labels.shape)
            # torch.Size([32, 80, 11, 30])
            # torch.Size([32, 80, 11, 1])
            # torch.Size([32, 80, 3, 1])

            encoder_inputs = encoder_inputs.transpose(-1, -2)  # (B, N, T, F)
            # print(encoder_inputs.shape)
            # torch.Size([32, 80, 30, 11])

            decoder_inputs = decoder_inputs.transpose(-1, -2)[:, :, :, :3]  # (B, N, T, 1)
            # print(decoder_inputs.shape)
            # torch.Size([32, 80, 1, 3])
            labels = labels.transpose(-1, -2)
            # print(labels.shape)
            # torch.Size([32, 80, 1, 3])
            decoder_start_inputs = decoder_inputs[:, :, :1, :]  # 32 80 1 3
            decoder_input_list = [decoder_start_inputs]
            # print(decoder_start_inputs.shape)

            # print(len(decoder_input_list))
            optimizer.zero_grad()

            outputs, encoder_refill, encoder_prob = net(encoder_inputs, decoder_inputs, _mean, _std)
            ...
            # torch.Size([32, 80, 1, 3])
            # torch.Size([32, 80, 30, 3])
            encoder_prob = encoder_prob.permute(0, 1, 4, 2, 3)
            encoder_label = encoder_inputs[:, :, :, :3].unsqueeze(-1).permute(0, 1, 4, 2, 3)
            E = EvidentialLossSumOfSquares()
            E1 = EvidentialLoss_encoder()
            # print(encoder_inputs[:, :, :, :3].shape)
            # print('encoder_prob', encoder_prob.shape)
            # print('encoder_label', encoder_label.shape)
            # print(encoder_prob[0,0,:,0,0])
            # print(encoder_label[0,0,:,0,:])
            # print(encoder_inputs[3, 3, 0, 0])
            # print(encoder_inputs[:, :, :, :3])
            value = encoder_label + 1e-8
            # encoder_prob = encoder_prob + 1e-8
            # print(value[0,0,:,0,:])
            loss = criterion(outputs, labels,mask_matrix, type) \
                # criterion(encoder_refill, encoder_inputs[:, :, :, :3], mask_matrix, type) \
                #    + E(prob, labels, mask_matrix, type, DEVICE) * 0.1 \

                # + E1(encoder_prob, value, mask_matrix, type, DEVICE) * 0.01
            # 两部分的损失函数



            print('E', E(prob, labels, mask_matrix, type, DEVICE))
            print('E1', E1(encoder_prob, value, mask_matrix, type, DEVICE))
            #
            loss.backward()
    ...
    predict_main(net, params_path, best_epoch, test_loader, test_target_tensor, _mean, _std, mask_matrix, mask_or,
                 'test')
    ...
    for epoch in range(epochs, epochs + fine_tune_epochs):
        ...
        for _, batch_data in enumerate(train_loader):
            ...
            predict_length = labels.shape[2]  # 80
            ...
            for _ in range(predict_length):
                ...
                predict_output, out = net.decode(decoder_inputs, encoder_output)
                prob = out
                decoder_input_list = [decoder_start_inputs, predict_output]

            loss = criterion(predict_output, labels, mask_matrix, 'train')  \
                   # criterion(encoder_refill,encoder_inputs[:, :, :, :3],mask_matrix, 'train') \
                   # + E(prob, labels, mask_matrix, type, DEVICE) * 0.1
                # (criterion(predict_output, labels, mask_matrix, 'train') +
    ...
    folder_name = 'refill'  # 您可以替换为您想要的文件夹名称
    os.makedirs(folder_name, exist_ok=True)

    torch.save(mask_matrix, os.path.join(folder_name, 'mask_matrix'))
    torch.save(encoder_refill, os.path.join(folder_name, 'encoder_refill'))
    torch.save(encoder_inputs, os.path.join(folder_name, 'encoder_inputs'))
    torch.save(encoder_prob, os.path.join(folder_name, 'encoder_prob'))
...
def predict_main(net, params_path, epoch, data_loader, data_target_tensor, _mean, _std, mask_matrix, mask_or, type):
    ...
    predict_and_save_results(net, data_loader, data_target_tensor, epoch, _mean, _std, params_path, mask_matrix,
                             mask_or, type)


if __name__ == "__main__":
    train_loader, _, val_loader, _, test_loader, test_target_tensor, _mean, _std, mask_matrix, mask_or = load_graphdata_normY_channel(
        graph_signal_matrix_filename, DEVICE, batch_size)

    net = make_model(DEVICE, num_layers, encoder_input_size, decoder_input_size, d_model, adj_mx, adj_phase,
                     mask_matrix, nb_head,
                     num_of_mhalf, points_per_mhalf, num_for_predict, len_input, dropout=dropout,
                     aware_temporal_context=aware_temporal_context,
                     SE=SE, TE=TE, kernel_size=kernel_size, smooth_layer_num=smooth_layer_num,
