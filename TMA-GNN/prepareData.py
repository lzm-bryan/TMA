...
def get_road_adj(filename):
    ...
    print('adj_phase', adj_phase.shape)
...
def build_road_state(relation_file, state_file, neighbor_node, mask_num, save_dir):
    ...
    print('11111')
    pre_ = [i for i in range(16)]
    print(mask_inter)
    # mask_inter = [6,9]
    mask_inter = random.sample(pre_,1)
    print(mask_inter)
    mask_inter = [5, 6, 9, 10, 0, 1, 12, 14]
    mask_inter = random.sample(pre_,1)
    ...
    for state_dic in state_file:
        ...
        # print(type(road_feature))
        # print(road_feature.shape)
        all_road_feature.append(road_feature)
        # print(type(all_road_feature))
        #
        # print(road_feature.shape)
        # print(len(all_road_feature))
        # print(mask_or)
    # print(len(all_road_feature))
    print('mask_or',mask_or)
    print('road_update',road_update)
    folder_name = ''
    torch.save(road_update, os.path.join(folder_name, 'road_update111111'))
    indices = [index for index, value in enumerate(road_update) if value == 2]
    print(indices)
    print('inter_in_roads', inter_in_roads)
    print('inter_dict_id2inter', inter_dict_id2inter)
    print('road_dict_road2id', road_dict_road2id)
...
def get_sample_indices(data_sequence, num_of_mhalf, label_start_idx, len_input, num_for_predict, points_per_mhalf):
    ...
    if num_of_mhalf > 0:
        mhalf_indices = search_data(data_sequence.shape[0], num_of_mhalf, label_start_idx, len_input, num_for_predict,
                                    points_per_mhalf)
    ...
    # print('-=',mhalf_sample.shape,target.shape)
    # print(mhalf_sample.shape)
    # print(target.shape)
    return mhalf_sample, target


def read_and_generate_dataset(graph_signal_matrix_filename, num_of_mhalf, len_input, num_for_predict, points_per_mhalf,
                              save=False):
    '''
...
if __name__ == "__main__":
    ...
    parser.add_argument("--config", default='configurations/TMA.conf', type=str,
    ...
    graph_signal_matrix_filename = 's' + str(points_per_mhalf) + '_p' + str(num_for_predict) + '_n' + str(
        neighbor_node) + '_m' + str(mask_num) + '.pkl'
    ...
    # print(relation_filename)
    # print(state_file_list)
    # print(neighbor_node)
    # print(mask_num)
    ...
    read_and_generate_dataset(graph_signal_matrix_filename, num_of_mhalf=1, len_input=30, num_for_predict=1,
                              points_per_mhalf=30, save=True)
